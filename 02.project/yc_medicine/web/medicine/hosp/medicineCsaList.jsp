...
<head>
    ...
    <script type="text/javascript">
        ...
        function deleteCsa(id)//处理记录的删除
        {
            var currPage = $("#currPage1").val();
            var totalPages = $("#totalPages1").val();
            confirm('你确定要删除吗？',
                    function () {
                        $.post("<%=path%>/medicine/hosp/csa!removeCsa.action?id=" + id,
                                function (json) {
                                    if (json.isSuccess == 1) {
        ...
        function toUpdateCsa(id, hid) {
            var url = 'csa!toUpdateCsa.action?id=' + id + "&hid=" + hid;
            var currPage = $("#currPage1").val();
            var totalPages = $("#totalPages1").val();
        ...
        function findList(currPage, totalPages ) {
            ...
            $("#dat2").val(date2);
            plan.show();
            $.post("<%=path%>/medicine/hosp/csa!findCsaList.action", data,
                    function (json) {
                        ...
                        $.each(list, function (i, obj) //对从action获取一个json进行遍历操作
                        {
                            ...
                            trs += "<td>" + obj[0] + "</td><td>" + cmtVal + "</td><td>" + obj[2] + "</td><td>" + obj[3]
                                    ...
                                    +"</td><td><a href='javascript:void(0)' onclick='toUpdateCsa(" + obj[0] + "," + obj[1] + ")' >修改<a></td>" +
                                    "<td><a href='javascript:void(0)' onclick='return deleteCsa(" + obj[0] + ")'>删除</a></td>";
                        });
                        if (trs == "") {
        ...
        //新增
        function addCsaRecord() {
            var url = 'csa!toUpdateCsa.action?hid=' + hid;
            var currPage = $("#currPage1").val();
            var totalPages = $("#totalPages1").val();
            url += "&currPage=" + currPage + "&totalPages=" + totalPages;
            location.href = url;
    ...
    <div class="mianm">
        <div class="mainmt" style="height:35px;">
            <p>
                ...
                <input type="button" onclick="addCsaRecord()" value="  新 增  ">
            </p>
        </div>
        <div class="mianmm">
            <table id="mytable">
