...
<head>
    ...
    <script type="text/javascript">
        ...
        function deleteWf(id)//处理记录的删除
        {
            var currPage = $("#currPage1").val();
            var totalPages = $("#totalPages1").val();
            confirm('你确定要删除吗？',
                    function () {
                        $.post("<%=path%>/medicine/hosp/wf!removeWf.action?id=" + id,
                                function (json) {
                                    if (json.isSuccess == 1) {
        ...
        function toUpdateWf(id, hid) {
            var url = 'wf!toUpdateWf.action?id=' + id + "&hid=" + hid;
            var currPage = $("#currPage1").val();
            var totalPages = $("#totalPages1").val();
        ...
        function findList(currPage, totalPages ) {
            ...
            $("#dat2").val(date2);
            plan.show();
            $.post("<%=path%>/medicine/hosp/wf!findWfList.action", data,
                    function (json) {
                        ...
                        tables += "<th>id</th><th>给药剂量</th><th>合用胺碘酮</th><th>CYP2C9基因型</th><th>VKORC1基因型</th>" +
                                "<th>上次给药后血液浓度</th><th>预测血液浓度</th>" +
                        ...
                        $.each(list, function (i, obj) //对从action获取一个json进行遍历操作
                        {
                            ...
                            trs += "<td>" + obj[0] + "</td><td>" + obj[2] + "</td><td>" + admVal + "</td><td>" +cypVal
                                    + "</td><td>" + vkorcVal + "</td><td> " + obj[3] + "</td><td>" + obj[12]
                                    +"</td><td><a href='javascript:void(0)' onclick='toUpdateWf(" + obj[0] + "," + obj[1] + ")' >修改<a></td>" +
                                    "<td><a href='javascript:void(0)' onclick='return deleteWf(" + obj[0] + ")'>删除</a></td>";
                        });
                        if (trs == "") {
        ...
        //新增
        function addWfRecord() {
            var url = 'wf!toUpdateWf.action?hid=' + hid;
            var currPage = $("#currPage1").val();
            var totalPages = $("#totalPages1").val();
            url += "&currPage=" + currPage + "&totalPages=" + totalPages;
            location.href = url;
    ...
    <div class="mianm">
        <div class="mainmt" style="height:35px;">
            <p>
                ...
                <input type="button" onclick="addWfRecord()" value="  新 增  ">
            </p>
        </div>
        <div class="mianmm">
            <table id="mytable">
