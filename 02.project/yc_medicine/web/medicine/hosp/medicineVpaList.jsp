...
<head>
    ...
    <script type="text/javascript">
        ...
        function deleteVpa(id)//处理记录的删除
        {
            var currPage = $("#currPage1").val();
            var totalPages = $("#totalPages1").val();
            confirm('你确定要删除吗？',
                    function () {
                        $.post("<%=path%>/medicine/hosp/vpa!removeVpa.action?id=" + id,
                                function (json) {
                                    if (json.isSuccess == 1) {
        ...
        function toUpdateVpa(id, hid) {
            var url = 'vpa!toUpdateVpa.action?id=' + id + "&hid=" + hid;
            var currPage = $("#currPage1").val();
            var totalPages = $("#totalPages1").val();
        ...
        function findList(currPage, totalPages ) {
            ...
            $("#dat2").val(date2);
            plan.show();
            $.post("<%=path%>/medicine/hosp/vpa!findVpaList.action", data,
                    function (json) {
                        ...
                        tables += "<th>id</th><th>药物剂型</th><th>给药日期</th><th>抽血日期</th>" +
                                "<th>给药剂量</th><th>给药频率</th><th>上次给药后血液浓度</th><th>预测血液浓度</th>" +
                        ...
                        $.each(list, function (i, obj) //对从action获取一个json进行遍历操作
                        {
                            ...
                            trs += "<td>" + obj[0] + "</td><td>" + formVal + "</td><td>" + obj[2] + "</td><td>" + obj[6]
                                    + "</td><td>" + obj[9] + "</td><td>" + rateval + "</td><td> " + obj[14] + "</td><td>" + obj[28]
                                    +"</td><td><a href='javascript:void(0)' onclick='toUpdateVpa(" + obj[0] + "," + obj[1] + ")' >修改<a></td>" +
                                    "<td><a href='javascript:void(0)' onclick='return deleteVpa(" + obj[0] + ")'>删除</a></td>";
                        });
                        if (trs == "") {
        ...
        //新增
        function addVpaRecord() {
            var url = 'vpa!toUpdateVpa.action?hid=' + hid;
            var currPage = $("#currPage1").val();
            var totalPages = $("#totalPages1").val();
            url += "&currPage=" + currPage + "&totalPages=" + totalPages;
            location.href = url;
    ...
    <div class="mianm">
        <div class="mainmt" style="height:35px;">
            <p>
                ...
                <input type="button" onclick="addVpaRecord()" value="  新 增  ">
            </p>
        </div>
        <div class="mianmm">
            <table id="mytable">
